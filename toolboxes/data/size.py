...
def obj_size(data, dot=2) -> str:
    ...
    :return: 格式化后的字符串
    """
    size = float(asizeof.asizeof(data))
    # 位 比特 bit
    if 0 <= size < 1:
    ...
    # 负数
        ...
        raise ValueError('{}() takes number than or equal to 0, but less than 0 given.'.format(obj_size.__name__))
    return human_size
